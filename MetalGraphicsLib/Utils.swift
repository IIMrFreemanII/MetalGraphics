...
public func forEachGridCell(_ gridSize: int2, _ cellSize: Float, _ spacing: Float, _ cb: (float2) -> Void) {
...
public func generateRandomArray(count: Int, range: ClosedRange<Int>) -> [Float] {
  let randomArray = Array(repeating: Float(), count: count)
  return randomArray.map { _ in Float(Int.random(in: range)) }
}
