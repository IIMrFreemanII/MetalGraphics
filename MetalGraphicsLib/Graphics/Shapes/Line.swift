public struct Line {
  ...
  public var bounds: BoundingBox2D {
    let minX: Float = min(start.x, end.x) - thickness
    let minY: Float = min(start.y, end.y) - thickness
    
    let maxX: Float = max(start.x, end.x) + thickness
    let maxY: Float = max(start.y, end.y) + thickness
